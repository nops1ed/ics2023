...
#define NR_PAGE 8
 ...
 static void * alloc_section_space(AddrSpace *as, uintptr_t vaddr, size_t p_memsz){
  size_t page_n = p_memsz % PGSIZE == 0 ? p_memsz / 4096 : (p_memsz / 4096 + 1);
  //size_t page_n = ((vaddr + p_memsz - 1) >> 12) - (vaddr >> 12) + 1;
  void *page_start = new_page(page_n);
  
  for (int i = 0; i < page_n; ++i)
    map(as, (void *)((vaddr) + i * PGSIZE), (void *)(page_start + i * PGSIZE), 1);
...
static uintptr_t loader(PCB *pcb, const char *filename) {
  ...
  for (int i = 0; i < ehdr.e_phnum; i++) {
    if (phdr[i].p_type == PT_LOAD) {
      ...
      fs_read(fd, (void *)((phdr[i].p_vaddr) + paddr), phdr[i].p_memsz);
      memset((void *)((phdr[i].p_vaddr) + phdr[i].p_filesz + paddr), 0, phdr[i].p_memsz - phdr[i].p_filesz);
      /*
      fs_lseek(fd, phdr[i].p_offset, SEEK_SET);
