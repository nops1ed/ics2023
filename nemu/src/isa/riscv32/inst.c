...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  /* need sext here. */
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc , s->dnpc = (src1 + imm)& ~1, ftrace(s, s->pc, s->dnpc));
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = src1 + imm);
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(rd) = SEXT((int32_t)(src1 + imm), 64));
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1 + imm, sizeof(word_t) / 2) , sizeof(word_t) * 4));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(src1 +imm, sizeof(word_t) / 2));
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw   , R, R(rd) = SEXT((int32_t)(src1 + src2), 64));
