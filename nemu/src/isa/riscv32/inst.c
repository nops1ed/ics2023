...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT_START();
  /* need sext here. */
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc , s->dnpc = 2 *imm + s->pc, ftrace(s, s->pc, s->dnpc));
  /* need sext here. */
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc , s->dnpc = (src1 + 2 * imm)& ~1, ftrace(s, s->pc, s->dnpc));
