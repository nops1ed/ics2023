...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  /* need sext here. */
  //INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc , s->dnpc = (src1 + SEXT(BITS(imm, 11, 0), 12))& ~1, ftrace(s, s->pc, s->dnpc));

  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc , s->dnpc = imm + s->pc, ftrace(s, s->pc, s->dnpc));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc , s->dnpc = src1 + imm, ftrace(s, s->pc, s->dnpc));
...
//INSTPAT("0000001 ????? ????? 101 ????? 00100 11", srliDis, I, /* Nop Instrction */);
  ...
  /* Need extended. */
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , R, s->dnpc = CR(0x341));
